...
export function InstagramSection() {
  ...
  // Only start loading the iframe when the section scrolls into view
  useEffect(() => {
  ...
  // If the embed hasn't loaded within 6s, treat it as failed (often blocked by
  // browser tracking protection / ad blockers).
  useEffect(() => {
    ...
    <section id="instagram" className="mx-auto max-w-7xl px-6 py-20 lg:px-10 lg:py-24">
      <div ref={containerRef} className="rounded-3xl border border-border/60 bg-cream/50 p-6 sm:p-10">
        ...
        {!embedFailed ? (
          ...
          <div className="mt-8 rounded-2xl border border-border/60 bg-background p-8 text-center">
            <p className="text-navy/70">
              Instagram's embedded feed couldn't load here (some browsers or extensions block it).
            </p>
            <a
              href={IG_URL}
              target="_blank"
            ...
            >
              <Instagram size={16} /> View our feed on Instagram
            </a>
          </div>
        )}
      </div>
